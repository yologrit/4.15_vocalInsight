...
interface LyricLineItemProps {
  ...
  onTechniqueHover: (type: string | null, lineIndex?: number, techIndex?: number) => void;
  onTechniquePin: (type: string | null, lineIndex?: number, techIndex?: number) => void;
              ...
              return groups.map((group, gIdx) => {
                ...
                if (group.length === 1) {
                  ...
                  return (
                    <span
                      key={gIdx}
                      onMouseEnter={() => onTechniqueHover(tech.type, index, line.techniques.filter(t => t.type !== "breath").indexOf(tech))}
                      onMouseLeave={() => onTechniqueHover(null)}
                      onClick={(e) => {
                        e.stopPropagation();
                        onTechniquePin(tech.type, index, line.techniques.filter(t => t.type !== "breath").indexOf(tech));
                      }}
                  ...
                  return (
                    <div
                      key={gIdx}
                      onMouseEnter={() => onTechniqueHover(transitionKey, index, gIdx)}
                      onMouseLeave={() => onTechniqueHover(null)}
                      onClick={(e) => {
                        e.stopPropagation();
                        onTechniquePin(transitionKey, index, gIdx);
                      }}
