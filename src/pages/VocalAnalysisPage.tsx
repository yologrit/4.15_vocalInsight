...
const VocalAnalysisPage = () => {
  ...
  const [selectedTechniqueKey, setSelectedTechniqueKey] = useState<string>("0-0");
  ...
  const [feedback, setFeedback] = useState<Record<string, { 
    ...
    "0-0": { helpful: false, notRight: false, helpfulCount: 128 },
    "1-0": { helpful: false, notRight: false, helpfulCount: 45 },
    "2-0": { helpful: false, notRight: false, helpfulCount: 256 },
    "2-1": { helpful: false, notRight: false, helpfulCount: 89 },
    "3-0": { helpful: false, notRight: false, helpfulCount: 34 },
    "4-0": { helpful: false, notRight: false, helpfulCount: 78 },
    "5-0": { helpful: false, notRight: false, helpfulCount: 142 },
    "6-0": { helpful: false, notRight: false, helpfulCount: 56 },
  });
  ...
  const handleLineSelect = (line: AnalysisLyricLine) => {
    setSelectedLine(line);
    const lineIdx = mockAnalysisData.indexOf(line);
    // Default to first technique of the line if available
    setSelectedTechnique(line.techniques[0] || null);
    setSelectedTechniqueKey(`${lineIdx}-0`);
  };

  const handleTechniqueSelect = (e: React.MouseEvent, tech: AnalysisTechnique, lineIdx: number, techIdx: number) => {
    e.stopPropagation(); // Prevent line selection from overriding
    setSelectedTechnique(tech);
    setSelectedTechniqueKey(`${lineIdx}-${techIdx}`);
  };

  const getAccuracyColor = (accuracy: number, alpha?: number) => {
    const safeAccuracy = Math.max(0, Math.min(100, accuracy));

    // Refined palette: primary-tinted blue-violet spectrum
    // High accuracy = cool indigo/primary, Low = warm muted rose
    const high = { r: 79, g: 70, b: 229 };    // primary indigo
    const mid = { r: 147, g: 130, b: 200 };    // muted lavender
    const low = { r: 190, g: 120, b: 145 };    // muted rose
  ...
  const getLyricGradientColor = (progress: number, alpha?: number) => {
    ...
    // From primary indigo (good) to muted rose (poor) — cohesive with design system
    const start = { r: 79, g: 70, b: 229 };   // primary indigo
    const end = { r: 190, g: 120, b: 145 };    // muted rose

    const blend = (a: number, b: number, t: number) => Math.round(a + (b - a) * t);

    const r = blend(start.r, end.r, safeProgress);
    const g = blend(start.g, end.g, safeProgress);
    const b = blend(start.b, end.b, safeProgress);
                            ...
                            {(() => {
                              ...
                              return sortedTechs.map((tech, tIdx) => {
                                ...
                                return (
                                  <button
                                    key={tIdx}
                                    onClick={(e) => handleTechniqueSelect(e, tech, idx, tIdx)}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-6 border-t border-border mt-6 space-y-4">
                      <div className="flex items-center gap-2 flex-nowrap">
                        <button
                          onClick={() => handleFeedback(selectedTechniqueKey, 'helpful')}
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                            feedback[selectedTechniqueKey]?.helpful
                              ? "bg-primary/10 text-primary border border-primary/25"
                              : "bg-accent/60 text-muted-foreground hover:bg-accent border border-transparent"
                          }`}
                        >
                          <ThumbsUp className={`w-3.5 h-3.5 ${feedback[selectedTechniqueKey]?.helpful ? "fill-current" : ""}`} />
                          有帮助 {feedback[selectedTechniqueKey]?.helpfulCount || 0}
                        </button>
                        <button
                          onClick={() => handleFeedback(selectedTechniqueKey, 'notRight')}
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                            feedback[selectedTechniqueKey]?.subOption
                              ? "bg-destructive text-destructive-foreground border border-destructive"
                              : feedback[selectedTechniqueKey]?.notRight
                              ? "bg-destructive/10 text-destructive border border-destructive/20"
                              : "bg-accent/60 text-muted-foreground hover:bg-accent border border-transparent"
                          }`}
                        >
                          <ThumbsDown className={`w-3.5 h-3.5 ${feedback[selectedTechniqueKey]?.notRight || feedback[selectedTechniqueKey]?.subOption ? "fill-current" : ""}`} />
                          不太对
                        </button>
                        <button
                          onClick={() => handleTechFavorite(selectedTechniqueKey)}
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all ml-auto ${
                            techFavorites[selectedTechniqueKey]
                              ? "bg-yellow-500/10 text-yellow-600 border border-yellow-500/20"
                              : "bg-accent/60 text-muted-foreground hover:bg-accent border border-transparent"
                          }`}
                        >
                          <Star className={`w-3.5 h-3.5 ${techFavorites[selectedTechniqueKey] ? "fill-current" : ""}`} />
                          收藏
                        </button>
                      </div>
                          ...
                          >
                            <div className="grid grid-cols-2 gap-3">
                              {["音高判断不对", "节奏判断不对", "技巧判断不对", "评价太严苛"].map((option) => (
                                <button
                                  key={option}
                                  onClick={() => handleSubOption(selectedTechniqueKey, option)}
                                  className={`h-10 rounded-xl text-[14px] font-semibold transition-all ${
                                    feedback[selectedTechniqueKey]?.subOption === option
                            ...
                            {feedback[selectedTechniqueKey]?.subOption && !feedback[selectedTechniqueKey]?.isSubmitted && (
                              <div className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
                                <p className="text-[13px] text-muted-foreground font-medium">请提供正确描述（可选）：</p>
                                <textarea
                                  value={feedback[selectedTechniqueKey]?.correctionText || ""}
                                  onChange={(e) => setFeedback(prev => ({
                                    ...prev,
                                    [selectedTechniqueKey]: {
                                      ...prev[selectedTechniqueKey],
                                ...
                                />
                                <button
                                  onClick={() => handleSubmitCorrection(selectedTechniqueKey)}
                            ...
                            {feedback[selectedTechniqueKey]?.isSubmitted && (
